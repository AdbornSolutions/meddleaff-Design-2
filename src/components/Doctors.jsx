...
function Doctors() {
  ...
  return <section id="doctors" className="w-full bg-white py-24 lg:py-32"><div className="mx-auto max-w-[1400px] px-6 md:px-8"><div className="flex flex-wrap items-end justify-between gap-8"><div className="max-w-2xl"><Reveal><SectionLabel>Meet Your Consultant</SectionLabel></Reveal><Reveal delay={0.08}><h2 className="mt-6 font-display text-[clamp(2.25rem,4.4vw,3.85rem)] font-light leading-[1.04] tracking-[-0.02em] text-brand-navy">
                Focused Expertise.
                <br /><span className="text-brand-green">Compassionate Care.</span></h2></Reveal></div>{doctors.length > 1 && <Reveal delay={0.14} className="hidden gap-3 md:flex"><button
    type="button"
    onClick={() => scrollBy(-1)}
  ...
  >{doctors.map(
    (doctor) => <li
      key={doctor.name}
      className={`group shrink-0 snap-start ${doctors.length === 1 ? "w-full max-w-[420px]" : "w-[78vw] sm:w-[46vw] lg:w-[24vw] lg:max-w-[340px]"}`}
    ><div className="relative aspect-[3/4] overflow-hidden rounded-[2rem] bg-brand-cream"><img
      src={doctor.image}
      alt={`${doctor.name} profile portrait`}
      loading="lazy"
      className="h-full w-full object-cover transition-transform duration-500 ease-premium group-hover:scale-[1.04]"
    /></div><div className="transition-transform duration-300 ease-premium group-hover:-translate-y-1.5"><p className="mt-6 text-[11px] uppercase tracking-label text-brand-green">{doctor.speciality}</p><h3 className="mt-2 font-display text-2xl font-light text-brand-navy">{doctor.name}</h3><p className="mt-1 text-sm text-ink-muted">{doctor.experience}</p><SiteLink
      to="/doctor-profile"
